    ...
    .prepare(
      `INSERT INTO listings
       (id, vehicle_id, seller_id, source_name, source_access, source_listing_id, url, title, status,
        price_amount, price_currency, mileage, exterior_color, photo_urls_json, title_status, latitude, longitude, location_label,
    ...
    .prepare(
      `UPDATE listings
       SET vehicle_id = ?, seller_id = ?, url = ?, title = ?, status = ?, price_amount = ?, price_currency = ?,
           mileage = ?, exterior_color = ?, photo_urls_json = ?, title_status = ?, latitude = ?, longitude = ?, location_label = ?, last_seen_at = ?, updated_at = ?
       WHERE id = ?`
    )
    .bind(
      vehicleId,
  ...
  await db
    .prepare(
      `INSERT INTO listing_snapshots
       (id, listing_id, captured_at, price_amount, price_currency, mileage, exterior_color, photo_urls_json, status, raw_title, raw_description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      crypto.randomUUID(),
